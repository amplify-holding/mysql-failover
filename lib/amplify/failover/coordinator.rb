  ...
  module Failover
    ...
    # has a * next to the currently active node.
      def active_master_pretty
        node_list = connected_master_ids.map do |id|
          if id == active_master_id
            "* #{id}"
          else
            "  #{id}"
          end
      ...
      def trigger_failover!(server_id)
        ...
        fail 'A fail over is already in progress' unless lock.lock(wait: false)
      ...
      def server_id_exists?(server_id)
        (connected_master_ids || []).include? server_id
      end
    end
  end
end
